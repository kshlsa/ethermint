...
SIMAPP = ./app
HTTPS_GIT := https://github.com/kshlsa/ethermint.git
...
docker-build:
	...
	docker create --name ethermint -t -i kshlsa/ethermint:latest ethermint
...
PACKAGE_NAME:=github.com/kshlsa/ethermint
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/kshlsa/ethermint/types"
	godoc -http=:6060
...
test-import:
	go test -run TestImporterTestSuite -v --vet=off github.com/kshlsa/ethermint/tests/importer
